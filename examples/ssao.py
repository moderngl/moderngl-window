...
class SSAODemo(OrbitDragCameraWindow):
    ...
    def on_render(self, time: float, frametime: float):
        ...
        self.g_buffer.use()
        self.geometry_program["mvp"].write(mvp)
        ...
        self.ssao_program["m_camera_inverse"].write(glm.inverse(camera_matrix))
        self.ssao_program["m_projection_inverse"].write(glm.inverse(projection_matrix))
        self.ssao_program["v_camera_pos"].value = camera_pos
        self.ssao_program["f_camera_pos"].value = camera_pos
        self.ssao_program["mvp"].write(mvp)
    ...
    def render_ui(self):
        ...
        _, self.material_properties[1] = imgui.slider_float(
            "diffuse", self.material_properties[1], 0.0, 1.0
        )
        _, self.material_properties[2] = imgui.slider_float(
            "specular", self.material_properties[2], 0.0, 1.0
        )
        _, self.material_properties[3] = imgui.slider_float(
            "specular exponent", self.material_properties[3], 1.0, 50.0
        )
